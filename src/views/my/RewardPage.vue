        ...
        <ion-header :translucent="true">
            <ion-toolbar color="primary">
                <ion-title style="display:inline-block">
                    <ion-icon :icon="arrowBack" @click="() => router.go(-1)" /><span class="ion-margin-start">Reward</span>
                </ion-title>
        ...
        <ion-content>
            <ion-grid>
                <ion-row class="border-bottom-ctm">
                    <ion-col>
                        <EasyDataTable :headers="headers" :items="rewardData" buttons-pagination rows-per-page="10">
                        </EasyDataTable>
                    </ion-col>
                </ion-row>
...
export default defineComponent({
    ...
    setup() {
        ...
        const headers: Header[] = [
            { text: "Reward Amount", value: "referralamount" },
    ...
    data(){
        return{
            currentUser:'',
            rewardData:[{"referralamount":0,"createddate":0}]
        }
    },
    async mounted() {
        let userid = localStorage.getItem('session_user') || '';
        await axios
            .get('http://localhost:5000/getMyReward/'+userid)
            .then(res => this.rewardData = (res.data.code == 200) ? res.data.data:[{"referralamount":0,"createddate":0}])
    },
  ionViewDidEnter() {
