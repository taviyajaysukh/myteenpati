<template>
    <ion-page>
        <ion-header :translucent="true">
            <ion-toolbar color="primary">
                <ion-title style="display:inline-block">
                    <ion-icon :icon="arrowBack" @click="() => router.go(-1)" /><span class="ion-margin-start">Bankcard
                    </span>
                </ion-title>
                <ion-button slot="end" @click="() => router.push('/payment/addbankcard')">
                    <ion-icon :icon="add" />
                </ion-button>
            </ion-toolbar>
        </ion-header>
        <ion-content>
            <ion-grid>
                <ion-row class="address_list" v-for="item in bankData" :key="item.mobilenumber">
                    <ion-col size="2">
                        <ion-label>
                    ...
                    <ion-col size="9">
                        <p class="font-class"> {{item.actualname}}</p>
...
import {
    IonContent, IonPage, IonHeader, IonModal, IonToolbar, IonGrid, IonRow, IonCol, IonIcon, IonTitle, IonButton, IonImg, IonLabel, IonFooter, actionSheetController, toastController
} from '@ionic/vue';
...
export default defineComponent({
    name: 'BankcardPage',
    components: {
        IonContent, IonPage, IonHeader, IonModal, IonToolbar, IonGrid, IonRow, IonCol, IonIcon, IonTitle, IonButton, IonImg, IonLabel, IonFooter, FooterPage
    },
    setup() {
    ...
    data() {
        return {
            bankData: [{ "_id": '', "actualname": '', "bankname": '', "bankaccount": 0, "address": '', "mobilenumber": 0 }],
            currentUser: '',
            strid: '',
            isOpen: false
        }
    ...
    methods: {
        ...
        async confirm() {
            await axios
                .delete('http://localhost:5000/deleteBankcard', { "data": { "id": this.strid } })
                    ...
                    buttons: [
                        {
                            ...
                            data: 'Data value',
                            handler: () => {
                                this.router.push('/payment/addbankcard?cardId=' + str1)
                            },
                        },
                        {
                            text: 'Delete',
